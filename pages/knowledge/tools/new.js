...
export default function NewTool() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) {
        router.push('/login?redirect=/knowledge/tools/new')
        return
      }
      setUser(user)
      supabase
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-8">添加工具</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition resize-none"
              placeholder="详细介绍工具功能、使用场景、优缺点等"
            />
          </div>
