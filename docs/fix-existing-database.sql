...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '已为 posts 表添加 user_id 字段';
  ELSE
    RAISE NOTICE 'posts 表已存在 user_id 字段';
  END IF;
END $$;

-- ========== 第二步：为 posts 表添加其他缺失字段 ==========
...
SELECT '✅ 数据库修复完成！所有字段和表已就绪。' AS status;